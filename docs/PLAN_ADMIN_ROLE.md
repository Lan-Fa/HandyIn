...
- [ ] `Layout.tsx`：按角色渲染导航/角色文案（管理员/教师/课代表）
- [ ] `App.tsx`：新增 `RequireRole` 路由守卫
- [ ] `Users.tsx`：角色下拉增加「管理员」+ badge 配色
- [ ] `Classes.tsx`：管理员（单个+批量建班+删除）/ 教师（已加入列表 + 加入班级对话框）两套 UI
- [ ] `Students.tsx` / `Assignments.tsx`：班级下拉随 `/classes` 角色过滤自动收紧
